...
- [ ] **استپ ۱۲ — Live Browser View + Element Picker (نکته ۱ صاحب پروژه — راهکار A) (دسته F1)**
...
- **ترتیب اجرای پیشنهادی:**
  1. پایه‌ای: **کامپایل سبز** (۲) → **Node-base/Linux** (۳) → **Docker** (۴)
  2. دو نکته‌ی صاحب پروژه (اولویت بالا چون صریح خواسته شده):
     - **n8n:** بهبود API (۱۴) → n8n Node (۱۵)
     - **تعامل با مرورگر:** UI پایه (۷-۸) → Live View + Picker (۱۲) → افزونه (۱۳)
  3. تکمیلی: باگ‌های منطقی (۵) → Zod (۶) → ادیتور بصری (۱۰) → بلوک‌های بیشتر (۱۱) → تست (۹)

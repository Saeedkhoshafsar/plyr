...
- **آخرین به‌روزرسانی این سند:** 2026-06-04 — پس از پایان استپ ۱۴ (بهبود API برای ادغام n8n — sync `wait`، HMAC webhook، Idempotency-Key، OpenAPI).
...
**استپ‌های تمام‌شده:** ۱، ۲، ۳، ۴، ۵، ۵.۵، ۶، ۷، ۸، ۹، ۱۰، ۱۱، ۱۲، ۱۳، ۱۴، ۱۶ ✅ _(استپ ۱۶ زودتر از ۱۲ انجام شد چون استپ ۱۲ به آن وابسته بود — طبق AGENT_RULE 3)_

**استپ بعدی برای شروع:** **استپ ۱۵ — n8n Community Node (نکته ۲ صاحب پروژه) (دسته F4)** — ساخت پکیج `n8n-nodes-automationbackend` (ساختار استاندارد n8n)، عملیات‌ها Run Workflow / Get Job Result / Create Schedule / Cancel Job، یک Trigger node برای دریافت webhookهای ما با **تأیید HMAC** (از `verifyWebhookSignature` در `src/utils/signature.ts` به‌عنوان مرجع منطق استفاده شود)، و Credentials = API Key + Base URL. مرجع قرارداد API: `docs/openapi.yaml`. ⚠️ پکیج n8n با TypeScript خودش build می‌شود؛ ساختار پوشه/`package.json` آن جدا از بک‌اند است (احتمالاً زیر `n8n-node/` یا مشابه). ⚠️ نکته‌ی CORS: `CORS_ALLOWED_ORIGINS` همچنان به‌صورت پیش‌فرض خالی است (same-origin)؛ برای n8n مرورگری باید origin آن ست شود — منطق در `src/index.ts` خطوط ~۷۴–۹۶ (هدر `Idempotency-Key` در استپ ۱۴ به allow-headers اضافه شد).

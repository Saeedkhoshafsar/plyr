...
- **آخرین به‌روزرسانی این سند:** 2026-06-04 — پس از پایان استپ ۱۸ (حالت Self-Hosted تک‌کاربره `DEPLOYMENT_MODE`، دسته H).
...
**استپ‌های تمام‌شده:** ۱، ۲، ۳، ۴، ۵، ۵.۵، ۶، ۷، ۸، ۹، ۱۰، ۱۱، ۱۲، ۱۳، ۱۴، ۱۵، ۱۶، ۱۷، ۱۸ ✅ _(استپ ۱۶ زودتر از ۱۲ انجام شد چون استپ ۱۲ به آن وابسته بود — طبق AGENT_RULE 3)_

**استپ بعدی برای شروع:** طبق ترتیب پیشنهادی بخش «یادداشت‌ها» در `PLAN.md` (پایه‌ای: کامپایل سبز ✅ → Self-Hosted ✅ → **Node-base/Linux (استپ ۳)** → Docker تک‌فرمانه (استپ ۴)). صاحب پروژه استپ بعدی را با «ادامه» مشخص می‌کند. ⚠️ همان قرارداد CRLF برای `src/**` (+`package.json` ریشه + `.env.example`) و LF برای `tests/**`/`docs/**`/`public/**`/`extension/**`/`n8n-node/**` رعایت شود (ویرایش `src/**` با اسکریپت پایتونِ binary-mode).

...
- **آخرین به‌روزرسانی این سند:** 2026-06-04 — پس از پایان استپ ۱۲ (Live Browser View + Element Picker).
...
**استپ‌های تمام‌شده:** ۱، ۲، ۳، ۴، ۵، ۵.۵، ۶، ۷، ۸، ۹، ۱۰، ۱۱، ۱۲، ۱۶ ✅ _(استپ ۱۶ زودتر از ۱۲ انجام شد چون استپ ۱۲ به آن وابسته بود — طبق AGENT_RULE 3)_

**استپ بعدی برای شروع:** **استپ ۱۳ — افزونه‌ی کمکی Chrome (نکته ۱ صاحب پروژه — راهکار B) (دسته F2)** — ساخت اسکلت افزونه‌ی Manifest v3 در پوشه‌ی `extension/`، Element Picker روی مرورگر واقعی کاربر (content script)، ضبط اکشن‌ها (کلیک/تایپ/ناوبری) و تبدیل به `steps`، ارسال امن به backend با API Key، مستند نصب. ⚠️ نکته: منطق سلکتورِ Picker (CSS path با `:nth-of-type` + `CSS.escape` و XPath) از قبل در `src/core/LiveBrowser.ts` (ثابت `PICKER_SCRIPT`) نوشته شده — می‌توان همان را به‌عنوان content script افزونه بازاستفاده کرد. فرمت گام‌ها همان `{action, params}` کاتالوگ مشترک `public/js/actions.js`.

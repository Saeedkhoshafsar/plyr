...
- **آخرین به‌روزرسانی این سند:** 2026-06-04 — پس از پایان استپ ۱۵ (n8n Community Node — پکیج `n8n-nodes-automationbackend` در `n8n-node/`).
...
**استپ‌های تمام‌شده:** ۱، ۲، ۳، ۴، ۵، ۵.۵، ۶، ۷، ۸، ۹، ۱۰، ۱۱، ۱۲، ۱۳، ۱۴، ۱۵، ۱۶ ✅ _(استپ ۱۶ زودتر از ۱۲ انجام شد چون استپ ۱۲ به آن وابسته بود — طبق AGENT_RULE 3)_

**استپ بعدی برای شروع:** **استپ ۱۷ — Workflow Storage (ذخیره/بازاجرا/مدیریت workflow) (دسته G2)** — مدل ذخیره‌سازی workflow در Redis (id, userId, name, steps, نسخه, زمان)، CRUD endpoint `POST/GET/PUT/DELETE /workflows`، و بازاجرا. (استپ ۱۶ قبلاً انجام شده و استپ‌های ۱۴ و ۱۵ مربوط به n8n کامل شدند.) مرجع: بخش «دسته‌ها» در `PLAN.md`. ⚠️ همان قرارداد CRLF برای `src/**` و LF برای `tests/**`/`docs/**`/`public/**` رعایت شود؛ پکیج `n8n-node/**` کاملاً مستقل و LF است و با تنظیمات/تست بک‌اند تداخل ندارد (tsconfig بک‌اند فقط `src/**`).

...
- **آخرین به‌روزرسانی این سند:** 2026-06-04 — پس از پایان استپ ۱۷ (Workflow Storage — ذخیره/بازاجرا/نسخه‌بندی workflow، دسته G2).
...
**استپ‌های تمام‌شده:** ۱، ۲، ۳، ۴، ۵، ۵.۵، ۶، ۷، ۸، ۹، ۱۰، ۱۱، ۱۲، ۱۳، ۱۴، ۱۵، ۱۶، ۱۷ ✅ _(استپ ۱۶ زودتر از ۱۲ انجام شد چون استپ ۱۲ به آن وابسته بود — طبق AGENT_RULE 3)_

**استپ بعدی برای شروع:** **استپ ۱۸ — حالت Self-Hosted تک‌کاربره (`DEPLOYMENT_MODE`) (دسته H) — اولویت بالا** — افزودن `DEPLOYMENT_MODE` (`single`/`multi`، پیش‌فرض `single`)؛ در حالت `single` خاموش‌کردن Quota/VIP/Plan/Level و یک `API_TOKEN` ساده (تولید تصادفی خودکار)، مخفی‌کردن endpointهای admin مدیریت کاربر، rate-limit سبک پیش‌فرض + هشدارهای امنیتی نصب، تست هر دو حالت + README. مرجع: بخش «دسته H» و استپ ۱۸ در `PLAN.md`. ⚠️ همان قرارداد CRLF برای `src/**` (+`package.json` ریشه + `.env.example`) و LF برای `tests/**`/`docs/**`/`public/**`/`extension/**`/`n8n-node/**` رعایت شود (ویرایش `src/**` با اسکریپت پایتونِ binary-mode).

...
- **آخرین به‌روزرسانی این سند:** 2026-06-04 — پس از پایان استپ ۱۶ (کانال زنده).
...
**استپ‌های تمام‌شده:** ۱، ۲، ۳، ۴، ۵، ۵.۵، ۶، ۷، ۸، ۹، ۱۰، ۱۱، ۱۶ ✅ _(استپ ۱۶ زودتر از ۱۲–۱۵ انجام شد چون استپ ۱۲ به آن وابسته است — طبق AGENT_RULE 3)_

**استپ بعدی برای شروع:** **استپ ۱۲ — Live Browser View + Element Picker (نکته ۱ صاحب پروژه — راهکار A) (دسته F1)** — حالا که کانال زنده (استپ ۱۶) آماده است، وابستگی برطرف شده. راه‌اندازی WS امن برای استریم مرورگر، CDP Screencast روی canvas، ارسال کلیک/تایپ کاربر، Element Picker برای تولید سلکتور. ⚠️ زیرساخت قابل‌استفاده‌ی مجدد از استپ ۱۶: `LiveServer` (هندلر `upgrade` روی مسیر `/live/ws`؛ برای مسیر جدید مرورگر یک هندلر `upgrade` دیگر اضافه کن یا path را در همان مالتی‌پلکس کن)، `authorizeLive` برای auth، و CSP الان `connect-src: 'self' ws: wss:` را اجازه می‌دهد. **نکته:** در sandbox مرورگر سروری بدون X server بالا نمی‌آید (هرچند با `Xvfb :99`+`DISPLAY=:99` سرور HTTP کامل بالا می‌آید و WS/SSE تست شد)؛ تست استریم واقعی مرورگر ممکن است نیاز به headless screencast داشته باشد.

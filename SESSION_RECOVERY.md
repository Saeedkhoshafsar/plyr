...
- **آخرین به‌روزرسانی این سند:** 2026-06-04 — پس از پایان استپ ۱۸ (حالت Self-Hosted تک‌کاربره `DEPLOYMENT_MODE`) و استپ ۱۹ (اسکریپت نصب تعاملی `install.sh`).
...
- استپ ۱۹: **اسکریپت نصب تعاملی `install.sh` (دسته H3)** _(۲۰۲۶-۰۶-۰۴)_. فایل `install.sh` (**LF**، `chmod +x`) در ریشهٔ پروژه: اسکریپت bash تعاملیِ تأییدمحور (`set -euo pipefail`، خروجی رنگی با تشخیص TTY). منوی انتخاب «روی چه چیزی نصب می‌کنی؟» با سه هدف: (۱) **Server (Docker)** → تشخیص `docker compose`/`docker-compose` و اجرای `up -d --build`؛ (۲) **Server (Node)** → `install_redis_native` (تشخیص apt/dnf/yum/pacman/brew با `maybe_sudo`) + `npm install` + Playwright (`install:browser` یا `install:browser:deps`) + `npm run build` + اجرا با PM2 (`ecosystem.config.js`) + `pm2 startup` اختیاری؛ (۳) **Client** → راهنمای Load-unpacked افزونهٔ `extension/` + `cd n8n-node && npm install && npm run build` + نصب اختیاری در `~/.n8n/custom`. تابع `ensure_env_file` فایل `.env` را از `.env.example` می‌سازد و یک `API_TOKEN` تصادفی (`tok_<48hex>` با node/openssl/urandom) را با `sed` سازگار GNU/BSD در آن می‌نویسد. پرچم‌ها: `--server-docker`/`--server-node`/`--client`/`-y|--yes`/`-h|--help`. اعتبارسنجی: `bash -n` سبز + **shellcheck سطح warning تمیز**. مستند در README (بخش «نصب تعاملی با install.sh»). نکته: این کار به‌عنوان استپ ۱۹ ثبت شد چون فراتر از ۱۸ استپ اولیه بود ولی مستقیماً شکاف H3 (نصب آسان) را پر می‌کند.
